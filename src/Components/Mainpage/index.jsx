...
const Mainpage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // Headers for API requests (only include Authorization if token is present)
                  ...
                  <CardActionArea onClick={() => handleCardClick(item.link)}>
                    <CardContent sx={{ padding: "30px" }}>
                      {React.cloneElement(item.icon, {
                        sx: { fontSize: 40, color: "#4b81a8", mb: 2 },
                      })}
            ...
            {[
              ...
              <Grid item xs={12} sm={4} key={index} textAlign="center">
                <CardContent>
                  <Typography
                    component="h1"
                    sx={{ fontSize: "60px", fontWeight: 700, color: "#4b81a8" }}
                  >
                    {loading ? "---" : item.count}
                  </Typography>
                  <Typography
                    variant="body1"
                    sx={{ fontSize: 18, color: "gray", letterSpacing: 1 }}
                  >
                    {item.label}
                  </Typography>
                </CardContent>
